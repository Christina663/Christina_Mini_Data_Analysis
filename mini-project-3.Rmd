...
answer1.1 <- refined_trees %>%
  filter(neighbourhood_name %in% c("DUNBAR-SOUTHLANDS", "SHAUGHNESSY", "RILEY PARK",  "KENSINGTON-CEDAR COTTAGE", "RENFREW-COLLINGWOOD") )  %>%
...
print(answer1.1)
ggplot(answer1.1, aes(common_name)) + geom_bar() + facet_wrap("neighbourhood_name") + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank())
```
...
```
If we would like to investigate the spread of Japanese trees across Vancouver regarding their ages, we would be interested in only the year they were planted. Thus the new column containing only the year planted would be useful and tidy.
<!----------------------------------------------------------------------------->
...
ggplot(answer1.2, aes(x = age, y = size)) + geom_jitter(color = "orange")
```
